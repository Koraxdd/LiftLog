...
export default function SetRow({ index, exerciseIndex, register, removeSet, canDelete, errors }: SetRowProps) {
    return (
        <div className="flex flex-col">
            <div className={clsx(
                "border-t border-subtle pt-3 grid grid-cols-[2rem_1fr_1fr] items-center gap-6 md:gap-20", canDelete && "grid-cols-[2rem_1fr_1fr_2rem]"
            )}>
                ...
                {canDelete && 
                    <Button 
                        ...
                        size="xs" 
                        className="hover:bg-card px-2 py-1" 
                        onClick={() => removeSet(index)}
                    >
                            <X size={20} className="text-[#EF4444]" />
                    </Button>}
            </div>
            {(errors.exercises?.[exerciseIndex]?.sets?.[index]?.reps || errors.exercises?.[exerciseIndex]?.sets?.[index]?.weight) && 
                <span className="text-[#EF4444] text-sm mt-3">
                    {errors.exercises?.[exerciseIndex]?.sets?.[index]?.reps?.message ||
                        errors.exercises?.[exerciseIndex]?.sets?.[index]?.weight?.message}
                </span>}
        </div>
    )
}